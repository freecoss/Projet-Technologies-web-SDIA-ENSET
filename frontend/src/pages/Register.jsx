...
export function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // Le backend renvoie l'utilisateur et le token ensemble
    ...
    <div className="flex justify-center items-center min-h-[70vh] px-4 animate-in fade-in duration-500">
      <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl shadow-bch7al-darkgray/5 border border-bch7al-darkgray/10 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-extrabold text-bch7al-navy mb-2">Rejoignez bch7al 🚀</h1>
          <p className="text-bch7al-darkgray/70">Créez un compte pour participer à la communauté.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-sm font-bold text-bch7al-navy ml-1">Nom d'utilisateur</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-bch7al-darkgray/40" />
              <Input 
                type="text" 
        ...
        <p className="text-center mt-8 text-bch7al-darkgray/80 text-sm">
          ...
          <Link to="/login" className="font-bold text-bch7al-blue hover:underline">
            Se connecter
          </Link>
        </p>
